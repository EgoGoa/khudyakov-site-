...
// минуты → автоматически собранная страница-КП (/offer) в фирменном стиле
// сайта: под сферу, задачу и бюджет клиента, с решением, кейсами и
// тарифами, чтобы он заказал, уже всё изучив.
//
// КП собирается прямо в браузере из ответов и данных сайта, без сервера и
...
export const VIBE_QUESTIONS: VibeQuestion[] = [
  // ---------- Общие ----------
  {
    ...
    react: (v) => `Отлично, собираю КП под ${DIRECTION_WORD[String(v)] ?? "задачу"}`,
  },
  {
    id: "sphere",
    kind: "chips",
    ...
    react: (v) => {
      ...
      return n > 0 ? `В этой сфере у нас ${n} ${plural(n, "проект", "проекта", "проектов")} — покажу лучшие в КП` : "Подберём кейсы из соседних сфер";
    },
  },
  {
    id: "company",
    ...
    hint: "Название компании или бренда — оно встанет в заголовок КП",
  ...
  },
  {
    ...
    hint: "От цели зависит, с чего начнётся решение в КП",
    options: [
  ...
  // ---------- Ветка: сайт ----------
  {
    ...
    hint: "Если не уверен — выбери ближайшее, подскажем в КП",
    options: [
