...
export const SMM_SEO_SECTIONS: SeoSection[] = [
  {
    title: "Чем SMM-сервис внутри продакшна отличается от обычного",
    body: "SMM чаще всего покупают отдельно от съёмки — контент для соцсетей приходит от фрилансера, который не видел бренд вживую. HUD.SERVICE ведёт соцсети той же командой, что снимает рекламные ролики: одни операторы, монтажёры и продюсер на проекте. Reels и сторис снимаются на том же оборудовании и в том же визуальном языке, что и остальной продакшн бренда.",
  },
  {
