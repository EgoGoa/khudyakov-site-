    ...
    <Frame>
      ...
      {groups.map(([name, n], i) => (
        <In key={name} at={5 + i}>
          ...
          <text x={184 + (i % 2) * 80} y={80 + Math.floor(i / 2) * 50} fill="#fff" fontSize="8" fontFamily="inherit">{name}</text>
  ...
  return (
    <Frame>
      ...
      <In at={1}>
        ...
        <text x="26" y="78" fill="rgba(255,255,255,0.45)" fontSize="7" letterSpacing="1.2" fontFamily="inherit">ШАБЛОН · НЕ МЕНЯЕТСЯ</text>
      </In>
      {[["Приветствие", 92], ["Оффер", 118], ["Призыв", 80]].map(([k, w], i) => (
        <In key={k as string} at={2 + i}>
          ...
          <text x="34" y={99 + i * 22} fill="rgba(255,255,255,0.75)" fontSize="7.5" fontFamily="inherit">{k as string}</text>
        </In>
      ))}
      <In at={5}>
        ...
        <text x="208" y="94" fill="var(--sp-from)" fontSize="7" letterSpacing="1.1" fontFamily="inherit">ТОН БРЕНДА</text>
        <text x="208" y="110" fill="#fff" fontSize="8" fontFamily="inherit">Меняем содержание,</text>
        <text x="208" y="123" fill="#fff" fontSize="8" fontFamily="inherit">а не голос</text>
      </In>
  ...
  return (
    <Frame>
      ...
      <In at={1}>
        ...
        <text x="49" y="110" textAnchor="middle" fill="#fff" fontSize="8" fontFamily="inherit">Шаблон</text>
      </In>
      {v.map(([seg, line], i) => (
        <In key={seg} at={2 + i}>
          ...
          <text x="150" y={72 + i * 34} fill="var(--sp-from)" fontSize="7" letterSpacing="1" fontFamily="inherit">{seg.toUpperCase()}</text>
          <text x="150" y={83 + i * 34} fill="#fff" fontSize="8" fontFamily="inherit">{line}</text>
        </In>
      ))}
    ...
    <Frame>
      ...
      {[["A", 38, false], ["B", 52, false], ["C", 96, true]].map(([k, h, win], i) => (
        <In key={k as string} at={1 + i}>
          ...
          <text x={68 + i * 90} y="172" textAnchor="middle" fill="rgba(255,255,255,0.6)" fontSize="8" fontFamily="inherit">Версия {k as string}</text>
          <text x={68 + i * 90} y={154 - (h as number)} textAnchor="middle" fill={win ? "var(--sp-from)" : "rgba(255,255,255,0.5)"} fontSize="9" fontWeight="700" fontFamily="inherit">{`${(h as number) / 10}%`}</text>
        </In>
      ))}
      <In at={5}>
        <rect x="222" y="60" width="104" height="20" rx="10" fill="rgba(255,255,255,0.08)" stroke="var(--sp-from)" strokeOpacity="0.6" />
        <text x="274" y="74" textAnchor="middle" fill="#fff" fontSize="7.5" fontFamily="inherit">Побеждает C</text>
      </In>
    ...
    <Frame>
      ...
      {src.map((name, i) => (
        <In key={name} at={1 + i}>
          ...
          <text x="38" y={78 + i * 32} fill="rgba(255,255,255,0.85)" fontSize="8" fontFamily="inherit">{name}</text>
      ...
      <In at={5}>
        ...
        <text x="202" y="82" fill="rgba(255,255,255,0.5)" fontSize="7" letterSpacing="1.2" fontFamily="inherit">ОДИН ДАШБОРД</text>
        {[0, 1, 2, 3, 4].map((j) => (
    ...
    <Frame>
      ...
      {[["Кабинет", "3,1%"], ["CRM", "2,4%"], ["Метрика", "4,0%"]].map(([k, v], i) => (
        <In key={k} at={1 + i}>
          ...
          <text x="26" y={78 + i * 30} fill="rgba(255,255,255,0.6)" fontSize="8" fontFamily="inherit">{k}</text>
          <text x="124" y={78 + i * 30} textAnchor="end" fill="rgba(255,255,255,0.45)" fontSize="9" fontFamily="inherit">{v}</text>
      ...
      <In at={5}>
        ...
        <text x="252" y="106" textAnchor="middle" fill="#fff" fontSize="14" fontWeight="700" fontFamily="inherit">2,9%</text>
        <text x="252" y="122" textAnchor="middle" fill="var(--sp-from)" fontSize="7" letterSpacing="1" fontFamily="inherit">ОБЩАЯ МОДЕЛЬ</text>
      </In>
  ...
  return (
    <Frame>
      ...
      <In at={4}>
        ...
        <text x="185" y="72" textAnchor="middle" fill="#fff" fontSize="7" fontFamily="inherit">выбилось из нормы</text>
      </In>
  ...
  return (
    <Frame>
      ...
      <In at={1}>
        ...
        <text x="26" y="80" fill="rgba(255,255,255,0.45)" fontSize="7" letterSpacing="1.1" fontFamily="inherit">ЦЕНА ЗАЯВКИ</text>
        <text x="26" y="112" fill="var(--sp-from)" fontSize="26" fontWeight="700" fontFamily="inherit">+38%</text>
      </In>
      <In at={2}>
      ...
      <In at={3}>
        ...
        <text x="178" y="80" fill="var(--sp-to)" fontSize="7" letterSpacing="1.1" fontFamily="inherit">ПОЧЕМУ</text>
      </In>
      {["В кабинете «Реклама 2» вырос", "клик, но упала доля целевых", "заявок — сменился креатив"].map((t, i) => (
        <In key={i} at={4 + i}>
          <text x="178" y={98 + i * 16} fill="#fff" fontSize="8" fontFamily="inherit">{t}</text>
        </In>
      ))}
  ...
  return (
    <Frame>
      ...
      {roles.map(([name, use], i) => (
        <In key={name} at={1 + i}>
          <text x="14" y={78 + i * 24} fill="rgba(255,255,255,0.8)" fontSize="8" fontFamily="inherit">{name}</text>
          ...
          <text x="326" y={78 + i * 24} textAnchor="end" fill={use < 0.4 ? "rgba(255,255,255,0.45)" : "var(--sp-from)"} fontSize="8" fontWeight="700" fontFamily="inherit">{`${Math.round(use * 100)}%`}</text>
        </In>
      ))}
    ...
    <Frame>
      ...
      {m.map((name, i) => (
        <In key={name} at={1 + i}>
          ...
          <text x="28" y={78 + i * 32} textAnchor="middle" fill="#06110c" fontSize="8" fontWeight="700" fontFamily="inherit">{i + 1}</text>
          <text x="44" y={78 + i * 32} fill="#fff" fontSize="8.5" fontFamily="inherit">{name}</text>
        </In>
      ))}
      <In at={5}>
        <rect x="200" y="62" width="126" height="90" rx="12" fill="rgba(255,255,255,0.03)" stroke="rgba(255,255,255,0.1)" strokeDasharray="4 4" />
        <text x="263" y="98" textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="8" fontFamily="inherit">Общий курс</text>
        <text x="263" y="112" textAnchor="middle" fill="rgba(255,255,255,0.35)" fontSize="8" fontFamily="inherit">про нейросети</text>
  ...
  return (
    <Frame>
      ...
      <In at={1}>
        ...
        <text x="26" y="80" fill="rgba(255,255,255,0.45)" fontSize="7" letterSpacing="1.1" fontFamily="inherit">ЗАДАЧА ИЗ РАБОТЫ</text>
        <text x="26" y="96" fill="#fff" fontSize="8" fontFamily="inherit">Ответить на заявку по смете</text>
      </In>
      <In at={2}>
      ...
      <In at={3}>
        ...
        <text x="26" y="147" fill="#fff" fontSize="8" fontFamily="inherit">Сотрудник делает с помощью AI</text>
      </In>
      <In at={4}>
        <rect x="180" y="62" width="146" height="98" rx="12" fill="rgba(255,255,255,0.04)" stroke="rgba(255,255,255,0.12)" />
        <text x="192" y="80" fill="rgba(255,255,255,0.45)" fontSize="7" letterSpacing="1.1" fontFamily="inherit">РАЗБОР</text>
      </In>
      {["Где промпт слабый", "Что можно автоматизировать", "Как проверить ответ"].map((t, i) => (
        <In key={t} at={5 + i}>
          <circle cx="194" cy={96 + i * 20} r="3" fill="var(--sp-from)" />
          <text x="204" y={99 + i * 20} fill="#fff" fontSize="7.5" fontFamily="inherit">{t}</text>
        </In>
      ))}
    ...
    <Frame>
      ...
      {w.map((h, i) => (
        <In key={i} at={1 + i}>
          ...
          <text x={50 + i * 56} y="170" textAnchor="middle" fill="rgba(255,255,255,0.5)" fontSize="7.5" fontFamily="inherit">{i === 0 ? "Старт" : `Неделя ${i}`}</text>
        </In>
      ))}
      <In at={6}>
        ...
        <text x="282" y="82" textAnchor="middle" fill="#fff" fontSize="7.5" fontFamily="inherit">пользуются каждый день</text>
      </In>
